...
agv_commanded = None # 마지막으로 받은 명령어 초기화
latest_command = "start" # 이전 명령어 초기화
latest_commanded = "start"
...
def process_agv_commands():
    global agv_command, st, latest_command, agv_commanded, latest_commanded
    while True:
        with command_lock:
            command = agv_command
            if command:
                if command == agv_commanded:
                    if command == 'left':
                        st=time.time()
                        latest_command = 'left'
                        agv_commanded = 'left'
            ...
            if agv_commanded:
                if command != agv_commanded:
                    if agv_commanded == 'left':
                        st=time.time()
                        latest_commanded = 'left'
                        mc.clockwise_rotation(3,0.1)
                        ed=time.time()
                        print("elapsed time:",ed-st)

                    elif agv_commanded == 'right':
                        st=time.time()
                        latest_commanded = 'right'
                        mc.counterclockwise_rotation(3,0.1)
                        ed=time.time()
                        print("elapsed time:",ed-st)

                    elif agv_commanded == 'floor':
                        if latest_commanded == 'left':
                            st=time.time()
                            mc.clockwise_rotation(3,0.1)
                            ed=time.time()
                            print("elapsed time(floor):",ed-st) 

                        elif latest_commanded == 'right':
                            st=time.time()
                            mc.counterclockwise_rotation(3,0.1)
                            ed=time.time()
                            print("elapsed time(floor):",ed-st)
