...
def open_gripper(mc):
    print('OPEN')
    mc.set_eletric_gripper(1)  # 그립퍼 전원 켜기
    mc.set_gripper_value(100, 30)  # 그립퍼 열기
    time.sleep(1)
...
def move_center_block(mc,box_center):
    ...
    mc.sync_send_coords([robot_x, robot_y, 250, -170, 2.29, 84.92], 30, 0)
...
def turn_gripper(mc):
    ...
    mc.send_angle(6, joint6_angle+angle, 30)
    time.sleep(1)

# 컨베이어 벨트에서 잡은 뒤에 컨베이어 벨트에 걸리는 것을 방지하는 위치로 이동하는 함수
def move_block_rear(mc):
    mc.sync_send_coords([(-80.9), (-102.3), 346, (-170), 17.77, 87.55], 50, 0)
    print('안정된 위치로 이동')
    time.sleep(1)
...
def move_sort_location_A(mc, block_count):
    ...
    if block_count == 0:
        ...
        # 세 번째 지점
        mc.sync_send_coords([128.7, (-333.7), 239.2, (-170), 0.71, (-159.42)], 20, 0)
...
def move_trash(mc):
    mc.sync_send_angles([10.45, (-60.11), 0.08, (-39.9), 92.54, 7.03], 30)
    print('버리는 위치로 이동')
    time.sleep(1)
...
completed = False
drop_state = False #블록이 카메라구역 안에 놓여있다:True
def cobot_1_sorting():
    # 이동 완료 플래그
    global completed
    global drop_state
...
if __name__ == '__main__':
    t1 = threading.Thread(target=cobot_1_sorting)
    t2 = threading.Thread(target=cobot_2_uploading)
    threads = [t1,t2]
    for t in threads:
        t.start()
    try:
        while True:
            print(drop_state)
            # pass  # Keep the main thread running until interrupted
    except KeyboardInterrupt:
        print("Keyboard interrupt detected. Joining threads...")
        for t in threads:
            t.join()
