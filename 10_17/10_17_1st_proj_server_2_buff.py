...
buffered_messages = []  # 수신된 메시지를 저장할 리스트
buffer_time_limit = 1.0  # 메시지를 저장할 최대 시간 (초)
...
def clear_buffer():
    ...
    while True:
        ...
        # 오래된 메시지를 처리합니다.
        buffered_messages[:] = [msg for msg in buffered_messages if current_time - msg[1] < buffer_time_limit]
        for msg in buffered_messages:
            process_message(msg[0])  # 메시지 처리
        buffered_messages.clear()  # 처리 후 버퍼 비우기
...
def control_agv():
    ...
    while True:
        try:
            server_socket.settimeout(1.1)  # 타임아웃 설정
            msg, addr = server_socket.recvfrom(BUFF_SIZE)
            client_addr = addr  # 클라이언트 주소 업데이트
            buffered_messages.append((msg, time.time()))  # 수신된 메시지와 수신 시간 저장
...
buffer_clear_thread = threading.Thread(target=clear_buffer)
