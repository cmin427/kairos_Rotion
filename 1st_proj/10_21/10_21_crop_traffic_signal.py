...
led_off_folder = r"C:\Users\aqsw6\Downloads\traffic_signal_crop (1)\traffic_signal_crop\light_none"
...
while True:
    ...
    _, _, v = cv2.split(cropped_hsv)
    v_adjusted = v - avg_v
    v_resized = cv2.resize(v_adjusted, (300,100))

    model = YOLO(r"C:\Users\aqsw6\Downloads\best (4).pt")
    results = model.predict(v_resized)

    #result에서 뽑아오기
    annotated_frame = results[0].probs
    ind = results[0].probs.top1
    print(results[0].names[ind])
    
    cv2.imshow('Detection', frame)
    cv2.imshow('cropped_hsv', v_resized)
